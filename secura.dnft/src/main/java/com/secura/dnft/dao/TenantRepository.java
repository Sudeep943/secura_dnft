...
public interface TenantRepository extends JpaRepository<Tenant, TenantId> {
    ...
    List<Tenant> findByAprmt_idAndFlatNo(@Param("aprmtId") String aprmt_id, @Param("flatNo") String flatNo);
    
    @Query("SELECT t FROM Tenant t WHERE t.tenantId = :tenantId AND t.flatNo = :flatNo AND t.aprmt_id = :aprmtId")
    Optional<Tenant> findByTenantIdAndFlatNoAndAprmt_id(@Param("tenantId") String tenantId, @Param("flatNo") String flatNo, @Param("aprmtId") String aprmt_id);
